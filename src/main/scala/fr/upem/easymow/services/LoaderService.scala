...
object LoaderService {
 ...
 // private def loadTondeuse(tondeuse : String)
  ...
  private def loadTondeuses(data : List[(String, String)], field : Option[Field]) : List[Option[Tondeuse]] = data.map{case (s1,s2)  => TondeuseFactory.buildTondeuse(PositionFactory.buildPosition(Integer.valueOf(s1.split(" ")(0)),Integer.valueOf(s1.split(" ")(1)), field), CardinalFactory.build(s1.split(" ")(2).charAt(0)), s2.toCharArray.map(c => CommandFactory.buildCommand(c)).toList)};
